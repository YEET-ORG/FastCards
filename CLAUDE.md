...
- Databases: local dev `kami` (ws://127.0.0.1:3000, start with `spacetime start`), production maincloud `kami-357rw` (identity `c200e31c…`). `npm run stdb:publish:local` / `stdb:publish:maincloud`; after schema changes run `npm run stdb:generate` to refresh `src/stdb/bindings`.
- Tests (`npm test`) auto-start a local SpacetimeDB if needed, publish `kami-test` fresh, and reset it per test via the gateway-gated `dev_reset` reducer. Test files must not run in parallel (shared DB — enforced in `vitest.config.ts`).
