...
export function SignInScreen() {
  const { privyError, retryPrivy, setSignUpName } = useAuth();
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.bg, paddingTop: insets.top + space.x40, paddingBottom: insets.bottom + space.xl }]}>
      ...
      <View style={styles.hero}>
        <AppText variant="hero" style={{ fontSize: 50, lineHeight: 56 }}>
          Your money, one conversation away.
        </AppText>
        <AppText variant="secondary" style={{ fontSize: 19, lineHeight: 26 }}>
          Cards, family controls, global money and AI — in one place.
        </AppText>
      </View>

      <View style={{ flex: 1, justifyContent: 'flex-end', gap: space.m, marginTop: space.x32 }}>
        {step === 'idle' ? (
          <Pressable
            onPress={startEmail}
