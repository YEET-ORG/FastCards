...
function DepositContentInner({ presentation = 'screen', scrollOffsetOut, scrollRef }: SheetScreenProps) {
  ...
  const scrollProps = {
    ...
    refreshControl: isSheet ? undefined : (
      <RefreshControl refreshing={syncing} onRefresh={() => void sync()} tintColor={colors.accent} />
    ),
  };
    ...
    <View style={styles.root}>
      {intent ? (
        <ScrollView {...scrollProps}>
          ...
          <SecondaryButton
            label={syncing ? 'Checking…' : 'Check for deposits'}
            loading={syncing}
            onPress={() => void sync()}
          />
        </ScrollView>
 ...
 */
export const DepositSheetContent = memo(DepositContentInner);
